...
def test_open_opus_work_api():
    ...
    url = f"https://api.openopus.org/work/list/composer/{composer_id}/genre/all.json"
    response = requests.get(url)
...
def test_weather_api():
    ...
    url = f"http://api.weatherapi.com/v1/current.json?key={WEATHER_API_KEY}&q=London&aqi=no"
    response = requests.get(url)
...
def test_gemini_api_weather_suggestion():
    ...
    prompt = (
        ...
        f"Consider selecting from works by these composers: {', '.join(composers)}. "
    ...
    assert any(composer.lower() in response_lower for composer in composers) or \
           weather_desc.lower() in response_lower or \
...
def test_youtube_search_url():
    ...
    search_query = f"{test_piece['composer_name']} {test_piece['title']} {test_piece.get('subtitle', '')}"
